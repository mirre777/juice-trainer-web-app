...
// Add these imports
import { ClientSelectorDialog } from "@/components/programs/client-selector-dialog"
...
export default function ReviewProgramClient({ importData }: ReviewProgramClientProps) {
  ...
  const [showClientSelectorDialog, setShowClientSelectorDialog] = useState(false)
  ...
  // Update the handleSendProgram function
  const handleSendProgram = async () => {
    if (!programState || !selectedClient) return

    try {
      setIsSaving(true)

      // Use the actual trainer ID and selected client ID
      const trainerId = "TRAINER_ID_PLACEHOLDER" // This should come from auth context
      ...
      if (!response.ok) {
        throw new Error(result.message || "Failed to send program")
      }

      console.log("Program sent successfully:", result.programId)

      // Show success message
      alert(`Program sent to ${selectedClient.name} successfully!`)
      ...
      alert(`Failed to send program: ${error.message}`)
    } finally {
      setIsSaving(false)
    }
  }

  // Update the Send to Client button click handler
  const handleSendToClientClick = () => {
    if (hasChanges) {
      alert("Please save your changes before sending the program to a client.")
      return
    }
    setShowClientSelectorDialog(true)
  }
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
          ...
          {/* Update the Send to Client button */}
          <Button
      ...
      {currentRoutines && currentRoutines.length > 0 ? (
              ...
              {expandedRoutines[routineIndex] && (
                <div className="p-4">
                  {routine.exercises && routine.exercises.length > 0 ? (
                    ...
                    <div>No exercises in this routine.</div>
                  )}
                </div>
              )}
            </div>
        ...
        <div className="text-gray-500">No routines for this week.</div>
      )}

      {/* Client Selector Dialog */}
      <ClientSelectorDialog
        open={showClientSelectorDialog}
        onOpenChange={setShowClientSelectorDialog}
        onClientSelect={(client) => {
          setSelectedClient(client)
          setShowSendProgramDialog(true)
        }}
        trainerId="TRAINER_ID_PLACEHOLDER" // This should come from auth context
      />

      {/* Update the Send Program Confirmation Dialog */}
          ...
          <div className="grid gap-4 py-4">
            <Card className="p-4">
              ...
              <div className="flex items-center text-sm text-gray-600 mb-1">
                <User className="h-4 w-4 mr-2" /> {selectedClient?.name || "No client selected"}
              </div>

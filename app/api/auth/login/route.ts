...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      // Continue with Firebase Auth even if Firestore fails
      console.log(`[API:login] ⚠️ Continuing with Firebase Auth despite Firestore error`)
    }
    ...
    if (user.hasFirebaseAuth) {
      ...
      // Legacy user handling remains the same...
      ...
      try {
        ...
        // Handle invitation code processing for auto-linked users
        if (invitationCode) {
          try {
            const { storeInvitationCode } = await import("@/lib/firebase/user-service")
            const storeResult = await storeInvitationCode(user.id, invitationCode)
            ...
            console.error(`[API:login] Error processing invitation for auto-linked user:`, inviteError)
          }
        }
        ...
        if (linkCheckError.code === "auth/user-not-found") {
          try {
            ...
            // Handle invitation code for migrated users
            if (invitationCode) {
              try {
                const { storeInvitationCode } = await import("@/lib/firebase/user-service")
                const storeResult = await storeInvitationCode(user.id, invitationCode)
                ...
                console.error(`[API:login] Error processing invitation for migrated user:`, inviteError)
              }
            }

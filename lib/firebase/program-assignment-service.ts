...
export const unassignProgramFromClient = async (trainerId: string, clientId: string, programId: string) => {
  try {
    ...
    await db.collection(`trainers/${trainerId}/clients/${clientId}/assignedPrograms`).doc(programId).delete()

    // Update program's assigned clients list
    const programRef = db.collection(`trainers/${trainerId}/programs`).doc(programId)
...
export const getClientPrograms = async (trainerId: string, clientId: string) => {
  try {
    ...
    const snapshot = await db.collection(`trainers/${trainerId}/clients/${clientId}/assignedPrograms`).get()
    ...
    for (const programId of programIds) {
      const programDoc = await db.collection(`trainers/${trainerId}/programs`).doc(programId).get()
      if (programDoc.exists) {

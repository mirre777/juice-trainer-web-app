...
export function SheetsImportDialog({ open, onOpenChange }: SheetsImportDialogProps) {
  ...
  // Handle dialog visibility logic
  useEffect(() => {
    if (!open) {
    ...
    img.onerror = () => {
      // Even if image fails to load, show the dialog
      setImageLoaded(true)
    }
  ...
  const handleOkThanks = () => {
    // Mark as seen in current session only
  ...
  const handleDontShowAgain = () => {
    ...
    // Also mark as seen in current session
    sessionStorage.setItem("hasSeenSheetsInstructions", "true")
    setShouldShowDialog(false)
    onOpenChange(false)
  }
  ...
  const handleClose = () => {
    // Same as "Ok Thanks" - mark as seen in current session only
    sessionStorage.setItem("hasSeenSheetsInstructions", "true")
    setShouldShowDialog(false)
    onOpenChange(false)
  }
